...
arr_par, arr_energy, arr_var, arr_est_energy, arr_est_var, arr_steps, arr_höf, arr_max_flag = vqe_adam_h2_test(eps_bern=0.1,delta=0.1,hamiltonian_coeff=g)

print(np.sum(arr_steps))
plt.plot(arr_par, arr_est_energy, '.-', label='Energy')
plt.plot(arr_par, arr_est_var, '.-', label='Variance')
plt.plot(arr_par, arr_max_flag, '.-', label='momentum')
plt.plot(arr_par, arr_höf, '.-', label='Epsilon')
plt.xlabel(r'$\theta$')
